 ...
 * Se eliminó la validación de auth (getCurrentUserWithProfile).
 */
 ...
 * Accesible públicamente tras la eliminación de la autenticación.
 */
export async function getCalibrationHistory(positionId) {
  ...
  const processed = (events || []).map((ev) => {
    ...
    return {
      ...
      observations_clean,
      supervisor_name,
      technician: { name: 'Faro Mantenimiento', role: 'Operador' }
    };
  });
...
/**
 * Guardado de certificado externo.
 */
export async function saveExternalCalibration(formData) {
  ...
  if (!positionId || !provider || !performedAt) return { ok: false, error: 'Faltan datos.' };
  if (!file || !(file instanceof File)) return { ok: false, error: 'PDF inválido.' };
  ...
  const { data: event, error: insertError } = await supabase
    ...
    .insert({
      ...
      external_cert_pdf_url: urlData?.publicUrl,
