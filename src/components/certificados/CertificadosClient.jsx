...
function PendingFooter({ row }) {
  ...
  function openVerificacion() {
    if (!row.id) return;
    window.dispatchEvent(new CustomEvent('open:verification-cert', {
      detail: {
        id: row.id,
        pos_mtto: row.pos_mtto,
        equipment_name: row.equipment_name,
  ...
  return (
    <div className="border-t border-neutral-100 bg-brand-failSoft/20 px-4 py-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={openVerificacion}
          className="flex-1 inline-flex items-center justify-center gap-1.5 px-2.5 py-2 rounded-lg border-2 border-neutral-800 text-neutral-800 text-[12px] font-bold hover:bg-neutral-100 transition"
        >
          <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
